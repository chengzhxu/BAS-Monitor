    ...
    $.fn.TimeSheet = function(opt){
        ...
        * */
        var initColHeads = function(){
            var colHeadHtml = '<tr><td rowspan="2">'+sheetOption.data.sheetHead.name+'</td><td colspan="24" style="text-align: center">上午</td><td colspan="24" style="text-align: center">下午</td></tr><tr>';
            for(var i=0,curColHead=''; i<=sheetOption.data.dimensions[1]; i++){
                if(i===0){
                    colHeadHtml += '';
                } else {
                    if (i%2===0) {
                        curColHead = '<td colspan="2" class="TimeSheet-head" style="width: 20px">'+(i-2)/2+'</td>';
                    } else{
                        curColHead = '';
                    }
                }
